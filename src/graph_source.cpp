	...
	while (gs_effect_loop(effect, "Solid")) {
		...
		if (background == 0) {
			...
			for (size_t i = 0; i < sizeof(heart_rate_zones) / sizeof(heart_rate_zones[0]); i++) {
				float top = height - (static_cast<float>(heart_rate_zones[i].max_hr - 50) / 260.0f) * height  * 2;
				float bottom = height - (static_cast<float>(heart_rate_zones[i].min_hr - 50) / 260.0f) * height * 2;

				gs_effect_set_color(gs_effect_get_param_by_name(effect, "color"), heart_rate_zones[i].color);
