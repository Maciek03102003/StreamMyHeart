...
std::vector<std::vector<bool>> faceMask(struct input_BGRA_data *frame, std::vector<struct vec4> &face_coordinates)
{
	...
	if (modelConfiguration.empty()) {
		obs_log(LOG_ERROR, "Failed to find face detection model configuration file");
		throw std::runtime_error("Failed to find face detection model configuration file");
	}
	std::string modelWeights =
		obs_find_module_file(obs_get_module("pulse-obs"), "res10_300x300_ssd_iter_140000_fp16.caffemodel");
	if (modelWeights.empty()) {
		obs_log(LOG_ERROR, "Failed to find face detection model weights file");
		throw std::runtime_error("Failed to find face detection model weights file");
	}

	// Load the pre-trained model
	cv::dnn::Net net = cv::dnn::readNetFromCaffe(modelConfiguration, modelWeights);

	obs_log(LOG_INFO, "DNN model loaded!!!!");
